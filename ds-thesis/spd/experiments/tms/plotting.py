...
class TMSAnalyzer:
    ...
    def diagnose_direction_vs_magnitude(self, eps: float = 1e-12) -> dict[str, float]:
        subnets = self.extract_subnets()  # (C, F, H)
        W = self.target_model.linear1.weight.T  # (F, H)

        sub_norms = torch.norm(subnets, dim=-1)  # (C, F)
        target_norms = torch.norm(W, dim=-1)  # (F,)
        cos = torch.einsum("C f h, f h -> C f", subnets, W) / (sub_norms * target_norms + eps)

        feat = torch.arange(W.shape[0])
        cos_idx, norm_idx = cos.argmax(dim=0), sub_norms.argmax(dim=0)

        return {
            "frac_differ": (cos_idx != norm_idx).float().mean().item(),
            "mmcs_cos": cos[cos_idx, feat].mean().item(),
            "mmcs_norm_ratio": (sub_norms[cos_idx, feat] / target_norms).mean().item(),
            "magnitude_cos": cos[norm_idx, feat].mean().item(),
            "magnitude_norm_ratio": (sub_norms[norm_idx, feat] / target_norms).mean().item(),
        }
...
class TMSPlotter:
    ...
    def print_analysis_summary(self) -> None:
        ...
        print(f"mmcs_cos: {diag['mmcs_cos']:.4f}, mmcs_norm_ratio: {diag['mmcs_norm_ratio']:.4f}")
        print(f"magnitude_cos: {diag['magnitude_cos']:.4f}, magnitude_norm_ratio: {diag['magnitude_norm_ratio']:.4f}")
