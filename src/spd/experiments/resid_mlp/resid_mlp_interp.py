...
from spd.models.components import EmbeddingComponent, Transformer, LinearComponent
...
def compute_spd_weight_neuron_contributions(
    ...
    #     dim=0,
    # )
    W_in_spd = torch.stack(
    [
        einops.einsum(
            components[f"layers.{i}.mlp_in"].A,  # [d_embed, C, 1]
            components[f"layers.{i}.mlp_in"].B,  # [C, 1, d_mlp]
            "d_embed C k, C k d_mlp -> d_embed C d_mlp",
        )
        for i in range(n_layers)
    ],
    dim=0,
...
def plot_subcomponent_norms(model, save_path=None):
    fig, axes = plt.subplots(
        ...
        figsize=(6 * len(model.components), 4),
        squeeze=False
    )

    with torch.no_grad():
        for i, (name, comp) in enumerate(model.components.items()):
